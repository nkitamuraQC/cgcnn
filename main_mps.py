...
parser.add_argument('--resume', default='./save/mymodel.pth', type=str, metavar='PATH',
...
# args.mps = not args.disable_cuda and torch.cuda.is_available()
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer):
    ...
    for i, (input, target, _) in enumerate(tqdm(train_loader, desc="Training Progress")):
        ...
        if args.mps:
            input_var = (Variable(input[0].to("mps"),
                         Variable(input[1].to("mps")),
                         input[2].to("mps"),
                         [crys_idx.to("mps") for crys_idx in input[3]])
        else:
        ...
        if args.mps:
            target_var = Variable(target_normed.to("mps"))
        else:
